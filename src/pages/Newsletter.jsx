...
import { Article, Source } from "@/api/entities";
...
import { Mail, Send, Eye, Loader2, ArrowLeft, PenSquare } from "lucide-react";
...
import { EditableNewsletterPreview } from "../components/newsletter/EditableNewsletterPreview"; // Adjusted path
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const RECENT_HOURS = 48; // Define constant for recent hours threshold

// Fonction partagée pour construire le HTML de l'email
async function buildNewsletterHTML(articles, governmentPublications, formData, selectedCategory, customSettings, chroniqueSummary) {
...
function NewsletterPreview({ articles, governmentPublications, formData, selectedCategory, customSettings, chroniqueSummary }) {
  const [previewHtml, setPreviewHtml] = React.useState('');

  React.useEffect(() => {
    const generatePreview = async () => {
        const finalHtml = await buildNewsletterHTML(articles, governmentPublications, formData, selectedCategory, customSettings, chroniqueSummary);
        setPreviewHtml(finalHtml);
    };
    generatePreview();
  }, [articles, governmentPublications, formData, selectedCategory, customSettings, chroniqueSummary]);
...
export default function NewsletterPage() {
  ...
  const [editableSummary, setEditableSummary] = useState({ title: '', text: '', speaker: '', customSpeakerInput: '' });
  ...
  // ✅ CORRECTION : Fonction de réorganisation avec gestion d'erreur
  const handleReorderArticles = async (result) => {
    if (!result.destination) return;

    const visibleArticles = filteredArticles.filter(a => !a.newsletter_excluded);
    const items = Array.from(visibleArticles);
    const [reordered] = items.splice(result.source.index, 1);
    items.splice(result.destination.index, 0, reordered);

    try {
      // Mettre à jour l'ordre de manière séquentielle pour éviter le rate limiting
      for (let i = 0; i < items.length; i++) {
        try {
          await Article.update(items[i].id, { newsletter_order: i });
          // Petit délai pour éviter le rate limiting
          if (i < items.length - 1) {
            await new Promise(resolve => setTimeout(resolve, 100));
          }
        } catch (err) {
          console.warn(`Erreur mise à jour article ${items[i].id}:`, err.message);
        }
      }

      toast({
        title: "Ordre mis à jour",
        description: `"${reordered.title.substring(0, 50)}..." a été déplacé`,
      });

      // Recharger après un court délai pour permettre à l'API de se stabiliser
      setTimeout(() => loadData(), 500);
    } catch (error) {
      console.error("Erreur réorganisation:", error);
      toast({
        title: "Erreur",
        description: "Impossible de réorganiser les articles.",
        variant: "destructive"
      });
    }
  };

  // ✅ CORRECTION : Fonction de masquage avec gestion d'erreur
  const handleToggleExcludeArticle = async (articleId) => {
    try {
      const article = categorizedArticles.find(a => a.id === articleId);
      if (!article) return;

      await Article.update(articleId, {
        newsletter_excluded: !article.newsletter_excluded,
        newsletter_order: null // Reset order when excluded/included
      });

      toast({
        title: article.newsletter_excluded ? "Article affiché" : "Article masqué",
        description: article.newsletter_excluded
          ? "L'article sera inclus dans le bulletin."
          : "L'article sera exclu du bulletin.",
      });

      await loadData();
    } catch (error) {
      console.error("Erreur toggle exclude:", error);
      toast({
        title: "Erreur",
      ...
      const articlesForDashboardFeed = allArticles.filter(article => {
        ...
        if (article.publication_date) {
            try {
                ...
                if (diffInHours <= RECENT_HOURS) isRecent = true;
            } catch (e) { /* ignore */ }
        }
  ...
  useEffect(() => {
    loadData(); // Call loadData here for initial loading
  ...
  // ✅ CORRECTION : Filtrage sécurisé avec tri par ordre personnalisé
  const filteredArticles = (selectedCategory === 'all' 
    ...
    .sort((a, b) => {
      // Tri par ordre personnalisé (nulls ou undefineds à la fin)
      const aOrder = a.newsletter_order !== null && a.newsletter_order !== undefined ? a.newsletter_order : Infinity;
      const bOrder = b.newsletter_order !== null && b.newsletter_order !== undefined ? b.newsletter_order : Infinity;
      
      if (aOrder !== bOrder) {
        return aOrder - bOrder;
      }
      
      // Si pas d'ordre personnalisé, tri par date (comportement actuel)
  ...
  // Articles qui seront réellement inclus dans le bulletin (non exclus)
  const visibleFilteredArticles = filteredArticles.filter(a => !a.newsletter_excluded);
  ...
  const handleSend = async () => {
    ...
    if (visibleFilteredArticles.length === 0 && filteredGovPublications.length === 0 && !hasChroniqueSummary) {
    ...
    try {
      ...
      // Utiliser visibleFilteredArticles pour la construction du HTML (uniquement les articles non exclus)
      const finalHtml = await buildNewsletterHTML(visibleFilteredArticles, filteredGovPublications, formData, selectedCategory, customSettings, editableSummary);
      ...
      // ✅ Réinitialiser les champs de gestion de manière séquentielle
      console.log("🧹 Réinitialisation des articles...");
      // Inclure tous les articles qui ont été chargés dans la vue pour réinitialiser newsletter_order et newsletter_excluded
      const articlesToReset = [...categorizedArticles, ...categorizedGovPublications];
      
      for (let i = 0; i < articlesToReset.length; i++) {
        try {
          await Article.update(articlesToReset[i].id, {
            newsletter_order: null,
            newsletter_excluded: false
          });
          // Petit délai pour éviter le rate limiting
          if (i < articlesToReset.length - 1) {
            await new Promise(resolve => setTimeout(resolve, 50));
          }
        } catch (err) {
          console.warn(`Erreur réinitialisation newsletter_order/excluded pour article ${articlesToReset[i].id}:`, err.message);
        }
      }
      ...
      if (selectedCategory === 'all') {
        ...
        for (const article of allPressArticles) {
          try {
            await Article.update(article.id, { categories: [], is_manually_categorized: false });
            await new Promise(resolve => setTimeout(resolve, 50));
          } catch (err) {
            console.warn(`Erreur réinitialisation catégories article ${article.id}:`, err.message);
          }
        }
        ...
        for (const publication of allGovPublications) {
          try {
            await Article.update(publication.id, { 
            ...
            console.warn(`Erreur réinitialisation catégories publication ${publication.id}:`, err.message);
          }
        }
        ...
        for (const article of articlesWithCategory) {
          try {
            ...
            console.warn(`Erreur réinitialisation catégories article (spécifique) ${article.id}:`, err.message);
          }
        }
        ...
        for (const publication of govPublicationsWithCategory) {
          try {
            ...
            console.warn(`Erreur réinitialisation catégories publication (spécifique) ${publication.id}:`, err.message);
          }
        }
      }
      
      const totalSentPress = visibleFilteredArticles.length;
  ...
  const totalContent = visibleFilteredArticles.length + filteredGovPublications.length;
        ...
        <div className="grid grid-cols-1 gap-8">
          ...
          <div className="space-y-6">
            <EditableNewsletterPreview
              articles={filteredArticles} // Pass all filtered articles (including excluded ones) for editing
              governmentPublications={filteredGovPublications}
              onReorderArticles={handleReorderArticles}
              onToggleExclude={handleToggleExcludeArticle}
              customSettings={customSettings}
            />

            <Card>
              <CardHeader>
                <CardTitle>📧 Aperçu final du courriel</CardTitle>
                <p className="text-sm text-gray-600">
                  Rendu tel qu'il sera reçu par les destinataires
                </p>
              </CardHeader>
              <CardContent>
                <NewsletterPreview 
                  articles={visibleFilteredArticles} // Pass only visible articles to the final preview
                  governmentPublications={filteredGovPublications}
                  formData={formData} 
                  selectedCategory={selectedCategory} 
                  customSettings={customSettings} 
                  chroniqueSummary={editableSummary}
                />
              </CardContent>
            </Card>
          </div>
