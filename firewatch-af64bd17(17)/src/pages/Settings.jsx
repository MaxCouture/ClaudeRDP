...
const DEFAULT_CLAUDE_PDQ_PROMPT = `Tu es un assistant parlementaire expert pour l'Assemblée nationale du Québec. Ta tâche est de résumer la transcription d'une période de questions.

Règles à suivre :
1. **Structure** : Regroupe les échanges par thème principal en utilisant une balise <h3>.
2. **Intervenants** : Pour chaque échange, identifie clairement qui pose la question et qui répond dans une balise <h4>. Mentionne le nom, le rôle (ex: chef de l'opposition officielle, premier ministre) et le parti si possible.
3. **Contenu** : Dans une balise <p>, résume de manière concise l'essence de la question et les points principaux de la réponse.
4. **Ton** : Reste neutre, factuel et informatif.
5. **Langue** : Rédige en français québécois.
6. **Format** : Retourne uniquement le HTML formaté, sans préambule ni conclusion. JAMAIS de balises markdown comme \`\`\`html.
7. **Sujets du jour** : Au tout début de ta réponse, liste EXACTEMENT 3 points principaux (pas plus, pas moins) qui résument les sujets abordés aujourd'hui, sous forme de liste à tirets simples. Ces points doivent être concis (max 20 mots chacun).

Format attendu :
- Premier sujet principal
- Deuxième sujet principal  
- Troisième sujet principal

[Puis le HTML structuré avec <h3>, <h4>, <p>]`;

const DEFAULT_CLAUDE_PDQ_OTTAWA_PROMPT = `Tu es un assistant parlementaire expert pour la Chambre des communes du Canada (Parlement fédéral à Ottawa). Ta tâche est de résumer la transcription d'une période de questions.

Règles à suivre :
1. **Structure** : Regroupe les échanges par thème principal en utilisant une balise <h3>.
2. **Intervenants** : Pour chaque échange, identifie clairement qui pose la question et qui répond dans une balise <h4>. Mentionne le nom, le rôle (ex: chef de l'opposition officielle, premier ministre, ministre) et le parti si possible.
3. **Contenu** : Dans une balise <p>, résume de manière concise l'essence de la question et les points principaux de la réponse.
4. **Ton** : Reste neutre, factuel et informatif.
5. **Langue** : Rédige en français.
6. **Format** : Retourne uniquement le HTML formaté, sans préambule ni conclusion. JAMAIS de balises markdown comme \`\`\`html.
7. **Sujets du jour** : Au tout début de ta réponse, liste EXACTEMENT 3 points principaux (pas plus, pas moins) qui résument les sujets abordés aujourd'hui, sous forme de liste à tirets simples. Ces points doivent être concis (max 20 mots chacun).
...
const DEFAULT_CLAUDE_CHRONIQUE_PROMPT = `Tu es un expert en résumé de chroniques politiques québécoises. Tu génères des résumés thématiques, concis et structurés à partir de transcriptions.

Consignes :
- Seuls les propos du chroniqueur sont pris en compte
- Ton neutre, informatif et fidèle au style de l'intervenant
- Chaque point formulé de manière claire et concise
- Retirer les éléments superflu (blagues, digressions)
- Regrouper les propos similaires sous un seul thème
- Chaque thème contient entre 2 à 4 phrases maximum
- Format de sortie : points de forme (•) avec thème en gras`;
...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const loadDeputes = async () => {
            ...
            try {
                ...
                toast({ 
                    title: "Erreur de chargement des députés", 
    ...
    useEffect(() => {
        const loadDeputesFederaux = async () => {
            ...
            try {
                const allDeputesFederaux = await DeputeFederal.list('-nom', 400); // 400 as there are more federal deputies
    ...
    const handleUpdateDeputeFederal = async (deputeId, updates) => {
        try {
            ...
            const allDeputesFederaux = await DeputeFederal.list('-nom', 400);
            setDeputesFederaux(allDeputesFederaux);
