...
<body style="margin: 0; padding: 0; background-color: {{newsletter_body_bg_color}}; font-family: {{newsletter_font_family}};">
  ...
  if (summaryPoints && summaryPoints.length > 0 && summaryPoints.some(point => point.trim() !== '')) {
    const pointsHtml = `
      <div style="margin-bottom: 30px; padding: 20px 25px; background-color: #f8f9fa; border-left: 4px solid #967150; border-radius: 8px;">
        <h3 style="margin: 0 0 12px 0; font-size: 20px; font-weight: 700; color: #967150; font-family: ${customSettings.newsletter_title_font_family || 'Georgia, serif'};">
          Sujets du jour
        </h3>
  ...
  // Remplacer les <h4> (intervenants) par des blocs avec photo
  pdqContentWithPhotos = pdqContentWithPhotos.replace(/<h4>(.+?)<\/h4>/g, (match, speakerFullText) => {
    ...
    const photoHtml = depute && depute.photoUrl 
      ? `<img src="${depute.photoUrl}" alt="${depute.nomComplet}" class="speaker-photo" style="width: 80px; height: 80px; border-radius: 50%; object-fit: cover; margin-right: 15px; border: 3px solid #967150;" />`
      : `<div class="speaker-photo" style="width: 80px; height: 80px; border-radius: 50%; background-color: #e5e7eb; margin-right: 15px; display: flex; align-items: center; justify-content: center; border: 3px solid #967150;"><span style="font-size: 32px; color: #9ca3af;">👤</span></div>`;
    
    return `
      <div style="display: flex; align-items: center; margin-top: 30px; margin-bottom: 15px;">
        ${photoHtml}
        <h4 class="speaker-name" style="margin: 0; font-size: 20px; font-weight: 600; color: #333333; font-family: ${customSettings.newsletter_title_font_family || 'Georgia, serif'};">
          ${speakerFullText}
        </h4>
      </div>
    `;
  ...
  // Styliser les titres de sujet <h3>
  pdqContentWithPhotos = pdqContentWithPhotos.replace(/<h3>(.+?)<\/h3>/g, (match, subjectText) => {
    return `
      <div style="margin-top: 40px; margin-bottom: 20px;">
        <hr style="border: none; height: 3px; background-color: #967150; width: 45px; margin: 0 0 15px 0;" />
        <h3 style="margin: 0; font-size: 24px; font-weight: 700; color: #967150; font-family: ${customSettings.newsletter_title_font_family || 'Georgia, serif'}; line-height: 1.3;">
          ${subjectText}
        </h3>
      </div>
    `;
  ...
  pdqContentWithPhotos = pdqContentWithPhotos.replace(/<p>/g, `<p style="font-size: 16px; line-height: 1.6; color: #333333; margin: 10px 0; font-family: ${customSettings.newsletter_font_family || 'Arial, sans-serif'};">`);
  pdqContentWithPhotos = pdqContentWithPhotos.replace(/<ul>/g, `<ul style="font-size: 16px; line-height: 1.6; color: #333333; margin: 10px 0 10px 20px; font-family: ${customSettings.newsletter_font_family || 'Arial, sans-serif'};">`);
...
export default function SendPdQPage() {
  ...
  const renderPdQForm = (pdqType) => {
    ...
    const formData = pdqType === 'quebec' ? formDataQuebec : formDataOttawa;
    const setFormData = pdqType === 'quebec' ? setFormDataQuebec : setFormDataOttawa;
