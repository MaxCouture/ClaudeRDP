...
export default function RetranscriptionPage() {
  ...
  const detectSpeakersFromHTML = useCallback((htmlContent, availableDeputes) => {
    ...
    const detected = Array.from(h4Elements).map((h4, index) => {
      ...
      const speakerName = (fullText.match(/^([^,:]+)/)?.[1] || fullText).trim();
      
      const normalizeName = (name) => {
        return name
          .toLowerCase()
          .replace(/^(m\.|mme|monsieur|madame|le|la)\s+/i, '') // Remove titles
          .trim();
      };
      ...
      // Create normalized versions of depute names for efficient matching
      ...
      // 1. Exact match by normalized full name
      foundDepute = deputeData.find(d => d.normalizedFullName === normalizedSpeakerName)?.depute;

      // 2. Match by normalized last name (e.g., "Mme Tremblay" -> "Chantal Tremblay")
      if (!foundDepute) {
        const speakerLastNameMatch = normalizedSpeakerName.match(/\b(\w+)$/); // Get last word as potential last name
        if (speakerLastNameMatch) {
          const speakerLastName = speakerLastNameMatch[1];
          foundDepute = deputeData.find(d => d.normalizedLastName === speakerLastName)?.depute;
        }
      }

      // 3. Broader inclusion check (detected in depute full name, or depute full name in detected)
      if (!foundDepute) {
    ...
    if (selectedFilter === FILTERS.QUESTIONS_QUEBEC) {
      const loadDeputes = async () => {
        try {
          ...
          setDeputesFederaux([]); // Clear federal when Quebec is selected
        } catch (error) {
      ...
      const loadDeputesFederaux = async () => {
        try {
          ...
          setDeputes([]); // Clear provincial when Ottawa is selected
        } catch (error) {
      ...
      setDeputes([]); // Clear both when other filters are selected
      setDeputesFederaux([]); // Clear federal too
    }
        ...
        if (selectedFilter === FILTERS.CHRONIQUE) {
            ...
            if (pdqQuebecSummary) {
                ...
                if (deputes.length > 0) {
                    ...
                    // If deputes not loaded yet, just detect without mapping
            ...
            if (pdqOttawaSummary) {
                ...
                if (deputesFederaux.length > 0) {
                    ...
                    // If deputesFederaux not loaded yet, just detect without mapping
  ...
  const handleProcessText = async () => {
      ...
      try {
          if (selectedFilter === FILTERS.BRUTE) {
              ...
              const lines = summaryText.split('\n').map(l => l.trim()).filter(l => l); // Enlever les lignes vides dès le début
              let result = [];
              let inList = false;
              ...
              for (let i = 0; i < lines.length; i++) {
                ...
                if (line.startsWith('<strong>') || line.match(/^<strong>/)) {
                  // Fermer liste précédente si nécessaire
                  if (inList) {
                    result.push('</ul>');
                    inList = false;
                  }
                  
                  // Ajouter un espacement si ce n'est pas le premier titre
                ...
                // POINT DE LISTE (commence par -, •, <li>, ou <ul>)
                if (line.match(/^[-•]\s+/) || line.startsWith('<li>') || line.startsWith('<ul>')) {
                  lastWasTitle = false;
                  
                  // Si c'est juste <ul> ou </ul>, on gère la liste
                  if (line === '<ul>') {
                    if (!inList) {
                      result.push('<ul>');
                      inList = true;
                  ...
                  // Ouvrir la liste si nécessaire
                  if (!inList) {
                    result.push('<ul>');
                    inList = true;
                  }
                  
                  // Nettoyer et formater le point
                ...
                // Ligne de texte normal après un titre → devient un point de liste
                if (lastWasTitle) {
                  if (!inList) {
                    result.push('<ul>');
                    inList = true;
              ...
              console.log('LOG: ✅ HTML final généré:', finalHtml.substring(0, 300));
