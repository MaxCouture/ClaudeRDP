            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={formData.mailchimp_audience_id} onValueChange={(value) => setFormData({...formData, mailchimp_audience_id: value})}>
                  ...
                  <SelectContent>
                    <SelectItem value={null}>Aucune</SelectItem> {/* Changed from null to empty string to match state init */}
...
export default function CategoriesKeywordsPage() {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      console.log('LOG: 📊 Chargement des catégories et mots-clés...');
      
      // Charger catégories et mots-clés
      const [categoryData, keywordData] = await Promise.all([
        Category.list().catch(err => {
          console.error('LOG: ❌ Erreur chargement catégories:', err);
          return [];
        }),
        Keyword.list().catch(err => {
          console.error('LOG: ❌ Erreur chargement mots-clés:', err);
          return [];
        })
      ]);
      
      console.log(`LOG: ✅ ${categoryData.length} catégories chargées`);
      console.log(`LOG: ✅ ${keywordData.length} mots-clés chargés`);
      
      setCategories(categoryData);
      setKeywords(keywordData);
      
      // Charger audiences Mailchimp (optionnel)
      try {
        const audienceResponse = await getMailchimpAudiences();
        const audienceData = audienceResponse?.data || [];
        console.log(`LOG: ✅ ${audienceData.length} audiences Mailchimp chargées`);
        setAudiences(audienceData);
      } catch (audienceError) {
        console.warn('LOG: ⚠️ Impossible de charger les audiences Mailchimp:', audienceError.message);
        setAudiences([]);
      }
      
    } catch (error) {
      console.error("LOG: ❌ Erreur globale chargement:", error);
      toast({ 
        title: "Erreur", 
        description: `Problème de chargement: ${error.message}`, 
        variant: "destructive" 
      });
    } finally {
      setIsLoading(false);
  ...
  const handleSaveCategory = async (formData) => {
    try {
      console.log('LOG: 💾 Sauvegarde catégorie:', formData);
      ...
      console.error('LOG: ❌ Erreur sauvegarde catégorie:', error);
      toast({ 
        title: "Erreur", 
  ...
  const handleDeleteCategory = async (id) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette catégorie et tous ses mots-clés ?")) {
      try {
        console.log('LOG: 🗑️ Suppression catégorie:', id);
        
        const categoryKeywords = keywords.filter(k => k.category_id === id);
        console.log(`LOG: 🗑️ Suppression de ${categoryKeywords.length} mots-clés associés`);
        ...
        console.error('LOG: ❌ Erreur suppression catégorie:', error);
        toast({ 
          title: "Erreur", 
  ...
  const handleAddKeyword = async (categoryId, keywordData) => {
    try {
      console.log('LOG: ➕ Ajout mot-clé:', { categoryId, keywordData });
      
      const newKeyword = {
        word: keywordData.word,
        category_id: categoryId,
        weight: keywordData.weight || 1, // Ensure a default weight
        description: keywordData.description || '' // Ensure a default description
      };
      
      await Keyword.create(newKeyword);
      toast({ title: "Mot-clé ajouté", description: `"${keywordData.word}" a été ajouté avec succès.` });
      loadData();
    } catch (error) {
      console.error('LOG: ❌ Erreur ajout mot-clé:', error);
      toast({ 
        title: "Erreur", 
        description: `Impossible d'ajouter le mot-clé: ${error.message}`, 
        variant: "destructive" 
      });
    }
  };
  ...
  const handleDeleteKeyword = async (keywordId) => {
    try {
      console.log('LOG: 🗑️ Suppression mot-clé:', keywordId);
      await Keyword.delete(keywordId);
      toast({ title: "Mot-clé supprimé", description: "Le mot-clé a été supprimé." });
      loadData();
    } catch (error) {
      console.error('LOG: ❌ Erreur suppression mot-clé:', error);
      toast({ 
        title: "Erreur", 
        description: `Impossible de supprimer: ${error.message}`, 
        variant: "destructive" 
  ...
  const handleUpdateKeyword = async (keywordId, keywordData) => {
    try {
      console.log('LOG: ✏️ Modification mot-clé:', { keywordId, keywordData });
      ...
      console.error('LOG: ❌ Erreur modification mot-clé:', error);
      toast({ 
        title: "Erreur", 
