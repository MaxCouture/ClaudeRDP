...
import { recategorizeALL } from "@/api/functions"; // NEW IMPORT
import { cleanupDuplicates } from "@/api/functions"; // NEW IMPORT
...
export default function Dashboard() {
  ...
  const [isRecategorizing, setIsRecategorizing] = useState(false); // NEW STATE
  ...
  const [isScanning48h, setIsScanning48h] = useState(false);
  ...
  const [scanResults, setScanResults] = useState(null); // NOUVEAU: Pour stocker les résultats du scan
  ...
  useEffect(() => {
    ...
    if (!isScanning24h && !isScanning48h && !isArchiving && !isRecategorizing && hasInitiallyLoaded.current) { 
      const interval = setInterval(() => {
        loadData();
      }, 15 * 60 * 1000); // 15 minutes
      return () => clearInterval(interval);
    }
  }, [isScanning24h, isScanning48h, isArchiving, isRecategorizing, loadData]); 
  ...
  async function handleProcessSources(scanHours = 25) {
    ...
    const isProcessingCurrentScan = scanHours === 25 ? isScanning24h : isScanning48h;
    const setIsProcessingCurrentScan = scanHours === 25 ? setIsScanning24h : setIsScanning48h;
    ...
    const totalToScan = allSources.length + 4; // +4 pour JdQ, JdM, Le Quotidien, Le Droit
    ...
    try {
      ...
      // --- 4. Traitement exhaustif des autres sources RSS avec paramètre de durée ---
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <Button onClick={handleArchiveAllArticles} disabled={isArchiving || isScanning24h || isScanning48h || isRecategorizing} variant="outline" className="border-orange-200 text-orange-700 hover:bg-orange-50">
            ...
            <div className="flex flex-col items-end">
              <Button
                ...
                disabled={isScanning24h || isScanning48h || isArchiving || isRecategorizing}
              >
                {isScanning24h ? (
            ...
            <div className="flex flex-col items-end">
              <Button
                onClick={() => handleProcessSources(48)}
                variant={isScanning48h ? "destructive" : "default"}
                className={isScanning48h ? "bg-green-600 text-white hover:bg-green-700 min-w-[150px]" : "bg-indigo-600 hover:bg-indigo-700 min-w-[150px]"}
                disabled={isScanning24h || isScanning48h || isArchiving || isRecategorizing}
              >
                {isScanning48h ? (
                  <div className="flex items-center justify-center">
