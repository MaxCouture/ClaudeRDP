...
import { ExternalLink, Calendar, Clock, Edit3, FileText, Loader2, Rss, Users, AlertTriangle } from "lucide-react";
              ...
              <div className="flex-1 space-y-2 min-w-0">
                ...
                {!isCompact && (
                  <>
                      ...
                      {isMultipleSources ? (
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className="flex items-center gap-1.5 py-1 px-2 border-purple-200 text-purple-700 bg-purple-50 font-medium w-fit shrink-0">
                            ...
                            {groupedSources.length} médias
                          </Badge>
                          <div className="flex flex-wrap gap-1">
                            {groupedSources.slice(0, 4).map((src, index) => (
                              <Badge key={`${src.name}-${index}`} variant="outline" className="text-xs border-gray-200 text-gray-600 bg-gray-50">
                                {src.name}
                                {src.count > 1 && <span className="ml-1 font-bold">×{src.count}</span>}
                              </Badge>
                            ))}
                            {groupedSources.length > 4 && (
                              <Badge variant="outline" className="text-xs border-gray-200 text-gray-600 bg-gray-50">
                                +{groupedSources.length - 4} autres
                              </Badge>
                            )}
                          </div>
                        </div>
                    ...
                    {/* Indicateur Presse Canadienne */}
                    {isMultipleSources && (
                      <div className="bg-purple-50 p-2 rounded-md border border-purple-200">
                        <p className="text-xs text-purple-700">
                          📰 <strong>Article syndiqué</strong> - Probablement de La Presse Canadienne, publié dans plusieurs médias
                        </p>
                      </div>
                    )}
