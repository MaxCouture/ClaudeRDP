...
export default function PdQContentEditor({ htmlContent, deputes, detectedSpeakers, onSpeakerChange, onContentChange }) {
  ...
  const handleSpeakerChange = (interventionId, newDeputeId) => {
    console.log('handleSpeakerChange appelé:', { interventionId, newDeputeId });
    
    // Trouver le député sélectionné
    const selectedDepute = deputes.find(d => d.id === newDeputeId);
    
    const updatedInterventions = interventions.map(intervention => {
      if (intervention.id === interventionId) {
        return {
          ...intervention,
          deputeId: newDeputeId,
          // CRITIQUE : Mettre à jour le speakerText avec le nom complet du député
          speakerText: selectedDepute ? selectedDepute.nomComplet : intervention.speakerText
        };
      }
      return intervention;
    });
    ...
    // Rebuilder immédiatement avec le nouveau nom
  ...
  const rebuildHTML = (interventionsList) => {
    ...
    const fullHtml = interventionsList
      ...
      .map(intervention => {
        ...
        // CRITIQUE : Utiliser le speakerText qui a été mis à jour avec le nom du député
    ...
    console.log('📝 HTML reconstruit:', fullHtml.substring(0, 200));
