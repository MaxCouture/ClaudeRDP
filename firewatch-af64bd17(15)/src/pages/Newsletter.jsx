...
<body style="margin: 0; padding: 0; background-color: {{newsletter_body_bg_color}}; font-family: {{newsletter_font_family}};">
  ...
  if (chroniqueSummary && chroniqueSummary.text) {
    ...
    // 2) BOÎTE TEXTE (sans bandeau à l’intérieur)
    const textBox = `
      <tr>
        <td style="padding: 0 40px;">
            ...
            <tr>
              <td style="padding: 30px 25px 30px 25px;">
                <div style="display:flex; align-items:center; margin:0 0 20px 0;">
                  <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/059d7b705_icon_cata2.png"
                       alt="Catapulte" width="23" height="23" style="display:block; width:23px; height:23px; margin-right:12px; border:0;" />
                  <div>
                    ...
                    ${ (chroniqueSummary.speaker === 'custom' ? (chroniqueSummary.customSpeakerInput || '') : (chroniqueSummary.speaker || '') )
                        ? `<p style="font-family:${customSettings.newsletter_font_family || 'Inter, sans-serif'}; font-size:14px; color:${customSettings.newsletter_chronique_text_color || '#FFFFFF'}; margin:5px 0 0 0; opacity:0.9;">
                             par ${chroniqueSummary.speaker === 'custom' ? (chroniqueSummary.customSpeakerInput || '') : (chroniqueSummary.speaker || '')}
                           </p>`
                        : '' }
                  </div>
                </div>
    ...
    // 3) (Optionnel) bandeau de fin, également hors de la boîte
    let bottomBannerRow = '';
    ...
    // Assemblage avec espaceurs "frères" (fiables partout)
...
export default function NewsletterPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    segment: "",
    introduction: "",
    fromName: "Catapulte Communication",
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // NOUVEAU : Charger le résumé du jour pour l'édition
        try {
          ...
          if (summaries.length > 0) {
            ...
            const loadedSpeaker = summaries[0].speaker || ''; // The actual speaker name from DB

            const predefinedSpeakers = ["Jonathan Trudeau", "Louis Lacroix", "Philippe Léger"];
            let initialSpeakerSelectValue = ''; // What the <Select> 'value' will be
            let initialCustomSpeakerInputValue = ''; // What the custom <Input> 'value' will be

            if (predefinedSpeakers.includes(loadedSpeaker)) {
              initialSpeakerSelectValue = loadedSpeaker;
            } else if (loadedSpeaker) {
              // It's a custom speaker name, so select 'custom' in the dropdown
              initialSpeakerSelectValue = 'custom';
              initialCustomSpeakerInputValue = loadedSpeaker; // And put the name in the custom input
            }

            setEditableSummary({
              title: loadedTitle,
              text: loadedText,
              speaker: initialSpeakerSelectValue, // This is for the Select
              customSpeakerInput: initialCustomSpeakerInputValue // This is for the custom Input
            });
          } else {
            // If no summary found for today, ensure state is reset or default
            ...
            setEditableSummary({ title: '', text: '', speaker: '', customSpeakerInput: '' }); // Reset or ensure default if error
        }
  ...
  const handleSend = async () => {
    // CORRECTION: Ne plus exiger de segment obligatoirement
    if (!formData.audience) {
      toast({ title: "Sélection manquante", description: "Veuillez sélectionner une audience.", variant: "destructive" });
      return;
    }

    if (filteredArticles.length === 0 && filteredGovPublications.length === 0) {
      toast({ title: "Aucun contenu", description: "Aucun article ou publication catégorisé à envoyer.", variant: "destructive" });
      return;
    }

    setIsSending(true);
    try {
      // NOUVEAU: Titre simplifié pour l'envoi
      const updatedSubject = `Bulletin d'information`;
      
      const finalHtml = await buildNewsletterHTML(filteredArticles, filteredGovPublications, formData, selectedCategory, customSettings, editableSummary); // NOUVEAU: passer editableSummary

      const { sendNewsletter: sendNewsletterFunction } = await import("@/api/functions");
      await sendNewsletterFunction({
        subject: updatedSubject,
        audience: formData.audience,
        segment_id: formData.segment || null, // Permet segment vide
        html_content: finalHtml,
        from_name: formData.fromName,
        reply_to: formData.reply_to,
      });
      
      // CORRECTION MAJEURE: Réinitialiser TOUS les articles de la catégorie envoyée
      ...
      toast({ 
        ...
        description: `Bulletin ${categoryText} envoyé avec ${totalSentPress} article${totalSentPress > 1 ? 's' : ''} de presse et ${totalSentGov} publication${totalSentGov > 1 ? 's' : ''} gouvernementale${totalSentGov > 1 ? 's' : ''}. Contenus réinitialisés.` 
      });
  ...
  if (categorizedArticles.length === 0 && totalGovContent === 0) {
      return (
          <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
              <Mail className="w-16 h-16 text-gray-400 mb-4" />
              <h2 className="text-xl font-semibold text-gray-700">Aucun contenu catégorisé disponible</h2>
              <p className="text-gray-500 mb-6">Veuillez d'abord catégoriser des articles ou publications.</p>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      {formData.audience && (
                        <SelectItem value={null}> {/* Use empty string for "no segment" */}
              ...
              <CardContent>
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button 
                      disabled={isSending || totalContent === 0 || !formData.audience} 
                    ...
                    >
                      {isSending ? (
                        ...
                        <>
                          ...
                          Envoyer le bulletin ({totalContent} éléments)
                          {formData.segment ? 
                            ` au segment "${segments.find(s => s.id === formData.segment)?.name}"` : 
                            (formData.audience ? ` à toute l'audience "${audiences.find(a => a.id === formData.audience)?.name}"` : "")
                          }
                        </>
                      )}
                    </Button>
                ...
                <p className="text-xs text-gray-500 mt-2 text-center">
                  After sending, content will be automatically reset.
                </p>
              </CardContent>
            </Card>
          </div>
