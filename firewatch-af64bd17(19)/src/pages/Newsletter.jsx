...
export default function NewsletterPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let descriptionText = `Bulletin ${categoryText} envoyé`;
      if (totalSentPress > 0 || totalSentGov > 0) {
        descriptionText += ` avec ${totalSentPress} article${totalSentPress > 1 ? 's' : ''} de presse et ${totalSentGov} publication${totalSentGov > 1 ? 's' : ''} gouvernementale${totalSentGov > 1 ? 's' : ''}`;
      }
      if (hasChroniqueSummary) {
        descriptionText += (totalSentPress > 0 || totalSentGov > 0) ? ' + résumé de chronique' : ' avec résumé de chronique';
      }
      descriptionText += '. Contenus réinitialisés.';
