...
export default function RetranscriptionPage() {
  ...
  const transcriptionIntervalRef = useRef(null); // This is kept for consistency but main logic now in keepAliveIntervalRef
    ...
    try {
      if (liveCaptureMode === LIVE_CAPTURE_MODES.MICROPHONE) {
        stream = await navigator.mediaDevices.getUserMedia({ 
          audio: {
            ...
            noiseSuppression: false,  // ✅ Désactiver pour éviter les artifacts
      ...
      const transcribeAccumulatedAudio = async () => {
        ...
        if (timeSinceLast < MIN_INTERVAL_MS) {
          ...
          console.log(`⏳ Attente: ${waitTime}s`);
          return;
        }
        
        if (chunksRef.current.length === 0) {
          console.log('⚠️ Aucun chunk');
          return;
        }
        
        await new Promise(resolve => setTimeout(resolve, 100));
        
        // ✅ Avec chunks de 1.5s, minimum 3 chunks = 4.5s d'audio
        if (chunksRef.current.length < 3) {
          console.log(`⏭️ Pas assez de chunks (${chunksRef.current.length})`);
          return;
        }
        ...
        if (audioBlob.size < 10000) {
          console.log('⏭️ Trop petit (< 10 KB)');
          statsRef.current.chunksIgnores++;
          chunksRef.current = []; // ✅ Vider même si ignoré
          return;
        }
        
        if (audioBlob.size > 24 * 1024 * 1024) {
          console.warn('⚠️ Trop gros (> 24 MB)');
          statsRef.current.chunksIgnores++;
          chunksRef.current = []; // ✅ Vider même si trop gros
          return;
        }
        ...
        // ✅ CRITIQUE : Créer une copie du nombre de chunks AVANT transcription
        const chunksCount = chunksRef.current.length;
        const blobSize = (audioBlob.size / 1024).toFixed(1);
        ...
        try {
          ...
          // ✅ CRITIQUE : TOUJOURS vider le buffer dans le finally
          console.log(`🗑️ Vidage FORCÉ du buffer (${chunksCount} chunks, ${blobSize} KB)`);
          chunksRef.current = [];
        }
        ...
        if (result.success) {
          ...
          console.error(`❌ Transcription échouée (${chunksCount} chunks, ${blobSize} KB):`, result.error);
        }
      };
      ...
      mediaRecorderRef.current.onstop = () => {
        console.log("📡 Enregistrement arrêté, nettoyage...");
        if (recordingStream) {
      ...
      mediaRecorderRef.current.start(1500);  // ✅ 1.5 secondes (meilleur compromis qualité/latence)
      ...
      }, TRANSCRIPTION_INTERVAL_MS); // Use the general transcription interval
