...
export default function RetranscriptionPage() {
  ...
  const captureAndTranscribe = async () => {
    ...
    try {
      ...
      if (keepAliveIntervalRef.current) clearInterval(transcriptionIntervalRef.current);
      ...
      try {
          if (selectedFilter === FILTERS.BRUTE) {
              ...
              if (error) {
                  ...
                  if (errorMessage.includes('credit balance is too low') || errorMessage.includes('insufficient credits')) {
                      throw new Error("CRÉDITS ANTHROPIC ÉPUISÉS"); // Custom error message for easier detection in catch block
                  }
              ...
              let cleanHtml = response.data?.summary_html || response.summary_html || '';
              cleanHtml = cleanHtml.replace(/```html\s*/g, '').replace(/```/g, '').trim();
          ...
          if (descriptionMessage.includes('CRÉDITS ANTHROPIC ÉPUISÉS') || descriptionMessage.includes('credit balance is too low') || descriptionMessage.includes('insufficient credits')) {
              toast({
                  ...
                  variant: "destructive",
                  duration: 15000 // Afficher plus longtemps
              });
