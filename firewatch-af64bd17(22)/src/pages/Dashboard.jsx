...
import { Newspaper, FileText, RefreshCw, Archive, Search, Grid, Shield, CheckCircle, XCircle, Loader2, Tag } from "lucide-react"; // Added Loader2, Tag
...
const RECENT_HOURS = 36; // ✅ CORRECTION: Retour à 36h pour afficher seulement les articles récents
...
export default function Dashboard() {
  ...
  // États pour les scans spécifiques
  const [isScanning24h, setIsScanning24h] = useState(false);
  const [isScanning75h, setIsScanning75h] = useState(false);
  ...
  // Fonction de rafraîchissement avec option de forcer
  ...
  const checkAndRunAutoScan = useCallback(async () => {
    try {
      ...
      if (!lastScanTime || (now - parseInt(lastScanTime)) > FIFTEEN_MINUTES) {
        ...
        // Assuming captageComplet() without params defaults to a reasonable short period
  ...
  useEffect(() => {
    const performInitialLoad = async () => {
        ...
        try {
            // NOUVEAU: Nettoyer les articles avec dates futures AVANT de charger
  ...
  useEffect(() => {
    ...
    if (!isScanning24h && !isScanning75h && !isArchiving && !isRecategorizing && hasInitiallyLoaded.current) { 
      const interval = setInterval(() => {
        loadData();
      }, 15 * 60 * 1000); // 15 minutes
      return () => clearInterval(interval);
    }
  }, [isScanning24h, isScanning75h, isArchiving, isRecategorizing, loadData]); 
  ...
  async function handleProcessSources(scanHours = 25) {
    // Dynamiquement selectionner le bon état et setter basé sur scanHours
    const isProcessingCurrentScan = scanHours === 25 ? isScanning24h : isScanning75h;
    const setIsProcessingCurrentScan = scanHours === 25 ? setIsScanning24h : setIsScanning75h;
    
    if (isProcessingCurrentScan) return; // Empêcher de cliquer plusieurs fois sur le même bouton de scan

    setIsProcessingCurrentScan(true); // Activer l'état de scan spécifique
    setScanStartTime(Date.now());
    setProcessedSourcesCount(0);
    setCurrentSourceName('Initialisation...');
    setScanResults(null); // Réinitialiser les résultats
    ...
    toast({
      title: `Scan ${scanHours}h lancé`,
      description: `${totalToScan} sources à analyser (${scanHours} dernières heures).`
    });
    ...
    try {
      ...
      try {
        const quotidienPromise = scrapeLeQuotidien();
        const timeoutPromise = new Promise((_, reject) =>
      ...
      try {
        const saguenayPromise = scrapeRadioCanadaSaguenay();
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Le scan Radio-Canada Saguenay a dépassé 30s et a été sauté.')), 30000)
        );

        const saguenayResponse = await Promise.race([saguenayPromise, timeoutPromise]);

        if (saguenayResponse.data?.created > 0) {
          articlesAddedInTotal += saguenayResponse.data.created;
          toast({
            title: `Radio-Canada Saguenay analysé`,
            description: `${saguenayResponse.data.created} nouveaux articles.`
          });
          await delay(1000);
          await loadData(true);
        }
      } catch (saguenayError) {
        console.error("Erreur Radio-Canada Saguenay:", saguenayError);
        toast({ title: "Alerte Scan Radio-Canada Saguenay", description: saguenayError.message, variant: "destructive" });
      }
      ...
      // Pass the scanHours parameter to captageComplet
      const captageResponse = await captageComplet({ scanHours });
      ...
      toast({
        title: `🎉 Scan ${scanHours}h terminé !`,
        description: `${articlesAddedInTotal} articles ajoutés, ${captageResponse.data?.articles_rejected || 0} rejetés.`,
        duration: 8000
      });
      ...
      setIsProcessingCurrentScan(false); // Désactiver l'état de scan spécifique
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <Button onClick={handleArchiveAllArticles} disabled={isArchiving || isScanning24h || isScanning75h || isRecategorizing} variant="outline" className="border-orange-200 text-orange-700 hover:bg-orange-50">
            ...
            <div className="flex flex-col items-end">
              <Button
                onClick={() => handleProcessSources(25)}
                variant={isScanning24h ? "destructive" : "default"}
                className={isScanning24h ? "bg-green-600 text-white hover:bg-green-700 min-w-[150px]" : "bg-blue-600 hover:bg-blue-700 min-w-[150px]"}
                disabled={isScanning24h || isScanning75h || isArchiving || isRecategorizing}
              >
                {isScanning24h ? (
                  <div className="flex items-center justify-center">
                    <div className="w-2 h-2 bg-white rounded-full mr-2 animate-pulse"></div>
                    <span>Scan... ({processedSourcesCount}/{totalActiveSources})</span>
                  </div>
                ) : (
                  <>
              ...
              {isScanning24h && scanStartTime && (
                <div className="mt-1 text-xs text-gray-600 max-w-[150px]">
            ...
            <div className="flex flex-col items-end">
              <Button
                onClick={() => handleProcessSources(75)}
                variant={isScanning75h ? "destructive" : "default"}
                className={isScanning75h ? "bg-green-600 text-white hover:bg-green-700 min-w-[150px]" : "bg-indigo-600 hover:bg-indigo-700 min-w-[150px]"}
                disabled={isScanning24h || isScanning75h || isArchiving || isRecategorizing}
              >
                {isScanning75h ? (
                  <div className="flex items-center justify-center">
