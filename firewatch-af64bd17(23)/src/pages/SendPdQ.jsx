...
export default function SendPdQPage() {
  ...
  const [formDataQuebec, setFormDataQuebec] = useState({
    ...
    audience: "",
    segment: "", // Changed initial value from null to "" for consistency with Select components
    fromName: "Catapulte",
  ...
  const handleSend = async (pdqType) => {
    ...
    try {
      ...
      await sendPdQNewsletterFunction({
        ...
        segment_id: (formData.segment && formData.segment !== "__all__") ? formData.segment : null, // Corrected logic for segment_id
  ...
  const renderPdQForm = (pdqType) => {
    ...
    const targetDescription = segmentNameForPopup 
      ...
      : (formData.segment === "__all__" && audienceNameForPopup ? `à toute l'audience "${audienceNameForPopup}"` : '');
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {formData.audience && (
                      <SelectItem value="__all__"> {/* Changed value from "" to "__all__" */}
