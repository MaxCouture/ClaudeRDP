...
import { Save, Loader2, Code, Palette, LayoutDashboard, Mail, RefreshCw, PenSquare, User, Shield, Users, Search, Upload, FileArchive, CheckCircle, AlertCircle, X } from "lucide-react";
import { UploadFile } from "@/api/integrations";
// import { processDeputePhotosZip } from "@/api/functions"; // This is no longer directly called
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <Tabs defaultValue="profile" className="space-y-6">
                ...
                {/* NOUVEL ONGLET UPLOAD ZIP */}
                <TabsContent value="upload-photos" className="space-y-6">
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <Upload className="w-5 h-5 text-indigo-600" />
                                Upload en masse des photos (Ottawa)
                            </CardTitle>
                            <CardDescription>
                                Téléversez un fichier .zip contenant les 344 photos des députés fédéraux
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            {/* Intégrer le contenu de UploadDeputePhotos ici */}
                            <UploadDeputePhotosContent />
                        </CardContent>
                    </Card>
                </TabsContent>
...
function UploadDeputePhotosContent() {
  ...
  // Fonction pour charger JSZip dynamiquement
  const loadJSZip = () => {
    return new Promise((resolve, reject) => {
      // Check if JSZip is already available on the window object
      if (window.JSZip) {
      ...
      script.src = 'https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js'; // CDN URL for JSZip
      script.onload = () => {
        // After script loads, check window.JSZip again
        if (window.JSZip) {
          resolve(window.JSZip);
        } else {
          reject(new Error('JSZip non chargé correctement après le script.'));
        }
      };
  ...
  const handleUpload = async () => {
    ...
    setProgress({ current: 0, total: 0, currentFile: "Chargement de JSZip..." }); // Updated progress message
    setResults(null);

    try {
      toast({
        title: "🚀 Traitement démarré",
        description: "Extraction et upload en cours...",
      });

      // Charger JSZip dynamiquement
      const JSZip = await loadJSZip();
      
      setProgress({ current: 0, total: 0, currentFile: "Extraction du ZIP..." });

      // Lire et extraire le ZIP côté client
      const arrayBuffer = await file.arrayBuffer();
      const zip = new JSZip();
      const zipContent = await zip.loadAsync(arrayBuffer);

      console.log('📂 Fichiers dans le ZIP:', Object.keys(zipContent.files).length);

      // Charger tous les députés fédéraux
      setProgress({ current: 0, total: 0, currentFile: "Chargement des députés..." });
      const allDeputes = await DeputeFederal.list('-nom', 350);
      console.log(`✅ ${allDeputes.length} députés chargés`);

      const allProcessingResults = [];
      
      const imageFiles = Object.entries(zipContent.files).filter(([fileName, fileEntry]) => {
      ...
      setProgress({ current: 0, total: imageFiles.length, currentFile: "" });
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        const [fileNameInZip, fileEntry] = imageFiles[i];
        const baseName = fileNameInZip.split('/').pop();
        ...
        try {
          ...
          console.log(`🔄 [${i + 1}/${imageFiles.length}] ${baseName}`);

          // Parser le nom de fichier (Nom_Prenom_...)
          ...
          if (parts.length < 2) {
            allProcessingResults.push({
              fileName: baseName,
              status: 'error',
              error: 'Format de nom de fichier invalide (attendu: Nom_Prenom_...)'
            });
            errorCount++;
            continue;
          }
          ...
          // Chercher le député
          const depute = allDeputes.find(d => 
            d.nom && d.prenom &&
            d.nom.toLowerCase() === nom.toLowerCase() && 
            d.prenom.toLowerCase() === prenom.toLowerCase()
          );

          if (!depute) {
            console.log(`⚠️ Député non trouvé pour: ${prenom} ${nom}`);
            allProcessingResults.push({
              fileName: baseName,
              status: 'not_found',
              searchedFor: `${prenom} ${nom}`
            });
            notFoundCount++;
            continue;
          }

          // Extraire l'image blob
          const imageBlob = await fileEntry.async('blob');
          const imageFile = new File([imageBlob], baseName, { type: `image/${baseName.split('.').pop()}` });

          // Upload l'image
          const { file_url } = await UploadFile({ file: imageFile });

          // Mettre à jour le député
          await DeputeFederal.update(depute.id, {
            photoUrl: file_url
          });

          console.log(`✅ ${depute.nomComplet}`);
          successCount++;

          allProcessingResults.push({
            fileName: baseName,
            status: 'success',
            matchedDepute: depute.nomComplet,
            photoUrl: file_url
          });

          // Petit délai toutes les 10 photos pour éviter le rate limiting
          if ((i + 1) % 10 === 0) {
            await new Promise(resolve => setTimeout(resolve, 500));
          }

        } catch (error) {
          console.error(`❌ Erreur pour ${baseName}:`, error);
          allProcessingResults.push({
            fileName: baseName,
            status: 'error',
            error: error.message
      ...
      setResults({ results: allProcessingResults, stats: {
        total: allProcessingResults.length,
      ...
      toast({
        ...
        description: `${successCount}/${allProcessingResults.length} photos mises à jour`,
      });

    } catch (error) {
      console.error("Erreur d'upload du ZIP:", error);
      toast({
        title: "❌ Erreur",
        description: error.message || "Impossible de traiter le fichier ZIP",
        variant: "destructive"
      });
    } finally {
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
        ...
        <p className="text-xs text-blue-700 mt-3">
          ...
          Les noms doivent être sans accents ni caractères spéciaux.
        </p>
      </div>
      ...
      <div
        ...
        onClick={() => document.getElementById('fileInputSettings').click()}
      >
        {file ? (
          <div className="space-y-3">
      ...
      {progress && (
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <p className="text-sm text-blue-700 truncate">{progress.currentFile}</p>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex gap-4 text-sm">
            <span className="text-green-600 font-medium">
              ✅ {results.stats.success} succès
            </span>
            <span className="text-orange-600 font-medium">
              ⚠️ {results.stats.notFound} non trouvés
            </span>
            <span className="text-red-600 font-medium">
              ❌ {results.stats.error} erreurs
            </span>
          </div>
            ...
            {results.results?.map((result, index) => (
              <div
                ...
                className={`p-3 rounded-lg border flex items-center justify-between ${
                  ...
                  result.status === 'not_found' ? 'bg-orange-50 border-orange-200' :
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {result.matchedDepute && (
                      <p className="text-xs text-gray-600">→ {result.matchedDepute}</p>
                    )}
                    {result.error && (
